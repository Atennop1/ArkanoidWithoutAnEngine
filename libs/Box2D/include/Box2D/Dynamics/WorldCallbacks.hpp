...
/// match up one-to-one with the contact points in Box2D::Manifold.
struct ContactImpulse
{
	float32 normalImpulses[maxManifoldPoints];
	float32 tangentImpulses[maxManifoldPoints];
	int32 count;
};
...
public:
	...
	virtual float32 ReportFixture(	Fixture* fixture, const Vec2& point,
